...
export function calculateMockResult(input: SpeicherInput): SpeicherResult {
  ...
  const recommendedSizeKwh = Math.round(Math.min(15, Math.max(5, pvSizeKwp * consumptionFactor)) * 2) / 2;
  
  // Mock: Self-consumption with battery
  // Typically increases by 20-35% with appropriately sized battery
  const selfConsumptionWith = Math.min(75, selfConsumptionWithout + 25 + (recommendedSizeKwh / pvSizeKwp) * 5);
  
  // Mock: Calculate savings
  const additionalSelfConsumption = (selfConsumptionWith - selfConsumptionWithout) / 100;
  const savedEnergyKwh = annualPvGenerationKwh * additionalSelfConsumption;
  const savingsPerKwh = ECONOMIC_PARAMS.electricityPriceEurKwh - ECONOMIC_PARAMS.feedInTariffEurKwh;
  ...
  const isViable = paybackYears <= 12 && paybackYears < ECONOMIC_PARAMS.batteryLifetimeYears;
  
  // Generate recommendation text
  const recommendation = generateRecommendation(isViable, paybackYears, recommendedSizeKwh);
...
function generateRecommendation(
  ...
  if (paybackYears > 20) {
    return `Bei Ihrem Verbrauchsprofil ist ein Stromspeicher wirtschaftlich nicht sinnvoll. Die Amortisationszeit von über ${Math.round(paybackYears)} Jahren übersteigt die erwartete Lebensdauer deutlich.`;
  }
