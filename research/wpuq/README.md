...
## Production Wasser/Wasser asset (not catalogued)
...
This JSON is a research production *candidate*. It is **not** a
`packages/heatpump-profile` catalogue default. The 24 robustness houses stay
under `processed/robustness/` with research ids (`ww-wpuq-2019-sfh{nn}-v1`)
and must not be catalogued as `field-cohort-representative` without review.
