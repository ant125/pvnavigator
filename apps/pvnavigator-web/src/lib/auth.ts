...
export function hubPostLoginLocation(nextRaw: unknown): string {
  return resolvePostLoginLocation(nextRaw, "/konto");
}

const SIGN_UP_ERROR_MESSAGES: Record<string, string> = {
  missing: "Bitte E-Mail und Passwort eingeben.",
  mismatch: "Die Passwörter stimmen nicht überein.",
  disabled: "Neue Registrierungen sind derzeit deaktiviert.",
  rate: "Zu viele Bestätigungs-E-Mails. Bitte versuchen Sie es in ein paar Minuten erneut.",
  existing: "Für diese E-Mail-Adresse existiert bereits ein Konto. Bitte melden Sie sich an.",
  weak: "Das Passwort ist zu kurz.",
  config: "Anmeldung ist nicht konfiguriert.",
  generic: SUPABASE_AUTH_FALLBACK,
};

export function signUpErrorFromQuery(raw: unknown): string | undefined {
  if (typeof raw !== "string") return undefined;
  return SIGN_UP_ERROR_MESSAGES[raw];
}

export function mapSupabaseAuthErrorToSignUpCode(
  error: Pick<Error, "message"> & { code?: string },
): string {
  const message = error.message.toLowerCase();
  const code = (error.code ?? "").toLowerCase();
...
export function hubSignUpErrorUrl(code: string): string {
  const url = new URL("/konto-erstellen", getHubOrigin());
  url.searchParams.set("error", SIGN_UP_ERROR_MESSAGES[code] ? code : "generic");
  return url.toString();
}

export function hubSignUpConfirmUrl(): string {
  const url = new URL("/konto-erstellen", getHubOrigin());
